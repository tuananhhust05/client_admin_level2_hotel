...
const Widget = ({ type }) => {
  ...
  useEffect(() => {
    const checkUser= async()=>{ 
        if(type == "user"){
          const res = await axios.get(`/users/countlistuserorderedbyownerid/${user._id}`); // gửi token để check 
          setAmount(res.data.count)
        }
        else if(type == "order"){
          const res = await axios.get(`/orders/countorderbyownerid/${user._id}`); // gửi token để check 
          setAmount(res.data.count)
        }
        else if(type == "revenue"){
          const res = await axios.get(`/orders/SumOrderByOwnerIdBefore/${user._id}`); // gửi token để check 
          setAmount(res.data.sum)
        }
        else if(type == "contract"){
          const res = await axios.get(`/orders/SumOrderByOwnerIdAfter/${user._id}`); // gửi token để check 
          setAmount(res.data.sum)
        }
      }
    try{
